...
export class ProductListComponent {
  ...
  getInStockCount()
  {
    //this.products.filter(inv => inv.is_in_inventory == true).length    
    return this.products.map(item => item.is_in_inventory).filter(inv => inv == true).length;
  }

  getOutOfStockCount()
  {
    //this.products.filter(inv => inv.is_in_inventory == false).length
    return this.products.map(item => item.is_in_inventory).filter(inv => inv == false).length;
  }
  ...
  {
    this.selectedFilterRadioButton = selectedValue;
  } 

  @Input()
  destroyProductDetail!: boolean;
  
  @Output()
  onProductClicked: EventEmitter<boolean> = new EventEmitter;
  
  OnClick()
  {
    this.destroyProductDetail = false;
    this.onProductClicked.emit(this.destroyProductDetail);
  }
}
